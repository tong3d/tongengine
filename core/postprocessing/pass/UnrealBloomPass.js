...
{
	if(TONG.LuminosityHighPassShader === undefined)
	{
		console.error("UnrealBloomPass relies on TONG.LuminosityHighPassShader");
	}
	if(TONG.CopyShader === undefined)
	{
		console.error("UnrealBloomPass relies on TONG.CopyShader");
	}
	...
	{
		var renderTarget = new TONG.WebGLRenderTarget(0, 0, parameters);
	...
	this.renderTargetBright = new TONG.WebGLRenderTarget(0, 0, parameters);
	this.renderTargetBright.texture.generateMipmaps = false;

	//Luminosity high pass material
	var highPassShader = TONG.LuminosityHighPassShader;
	this.highPassUniforms = TONG.UniformsUtils.clone(highPassShader.uniforms);
	this.highPassUniforms["luminosityThreshold"].value = (threshold !== undefined) ? threshold : 0.7;
	this.highPassUniforms["smoothWidth"].value = 0.01;
	this.materialHighPassFilter = new TONG.ShaderMaterial(
	{
	...
	for(var i = 0; i < this.nMips; i++)
	{
		...
		this.separableBlurMaterials[i].uniforms["texSize"].value = new TONG.Vector2(0, 0);
	}
	...
	this.compositeMaterial.uniforms["bloomTintColors"].value = [new TONG.Vector3(1, 1, 1), new TONG.Vector3(1, 1, 1), new TONG.Vector3(1, 1, 1), new TONG.Vector3(1, 1, 1), new TONG.Vector3(1, 1, 1)];
	...
	this.basic = new TONG.MeshBasicMaterial();
...
UnrealBloomPass.BlurDirectionX = new TONG.Vector2(1.0, 0.0);
UnrealBloomPass.BlurDirectionY = new TONG.Vector2(0.0, 1.0);
...
UnrealBloomPass.prototype.render = function(renderer, writeBuffer, readBuffer, delta, maskActive, scene, camera)
{
	...
	renderer.setClearColor(new TONG.Color(0, 0, 0), 0);
...
UnrealBloomPass.getSeperableBlurMaterial = function(kernelRadius)
{
	return new TONG.ShaderMaterial(
	{
		...
		uniforms:
		{
			...
			"texSize":
			{
				value: new TONG.Vector2(0.5, 0.5)
			},
			"direction":
			{
				value: new TONG.Vector2(0.5, 0.5)
