...
TONG.ShaderLib['water'] = {

  uniforms: TONG.UniformsUtils.merge( [
    TONG.UniformsLib[ "fog" ], {
      ...
      "textureMatrix" :   { type: "m4", value: new TONG.Matrix4() },
      "sunColor":         { type: "c", value: new TONG.Color(0x7F7F7F) },
      "sunDirection":     { type: "v3", value: new TONG.Vector3(0.70707, 0.70707, 0) },
      "eye":              { type: "v3", value: new TONG.Vector3(0, 0, 0) },
      "waterColor":       { type: "c", value: new TONG.Color(0x555555) }
    }
  ] ),
  ...
  fragmentShader: [    
    ...
    TONG.ShaderChunk[ "common" ],
    TONG.ShaderChunk[ "fog_pars_fragment" ],
...
TONG.Water = function (renderer, camera, scene, options) {
  
  TONG.Object3D.call(this);
  ...
  this.sunDirection = optionalParameter(options.sunDirection, new TONG.Vector3(0.70707, 0.70707, 0.0));
  this.sunColor = new TONG.Color(optionalParameter(options.sunColor, 0xffffff));
  this.waterColor = new TONG.Color(optionalParameter(options.waterColor, 0x7F7F7F));
  this.eye = optionalParameter(options.eye, new TONG.Vector3(0, 0, 0));
  this.distortionScale = optionalParameter(options.distortionScale, 20.0);
  this.noiseScale = optionalParameter(options.noiseScale, 1.0);
  this.side = optionalParameter(options.side, TONG.FrontSide);
  this.fog = optionalParameter(options.fog, false);
  
  this.renderer = renderer;
  this.scene = scene;
  this.mirrorPlane = new TONG.Plane();
  this.normal = new TONG.Vector3(0, 0, 1);
  this.cameraWorldPosition = new TONG.Vector3();
  this.rotationMatrix = new TONG.Matrix4();
  this.lookAtPosition = new TONG.Vector3(0, 0, -1);
  this.clipPlane = new TONG.Vector4();
  ...
  this.texture = new TONG.WebGLRenderTarget(width, height);
  this.tempTexture = new TONG.WebGLRenderTarget(width, height);
  
  var mirrorShader = TONG.ShaderLib["water"];
  var mirrorUniforms = TONG.UniformsUtils.clone(mirrorShader.uniforms);
  ...
  if ( !TONG.Math.isPowerOfTwo(width) || !TONG.Math.isPowerOfTwo(height) ) {
    this.texture.generateMipmaps = false;
    this.tempTexture.generateMipmaps = false;
  }
};

TONG.Water.prototype = Object.create(TONG.Object3D.prototype);

TONG.Water.prototype.renderWithMirror = function (otherMirror) {
...
TONG.Water.prototype.updateTextureMatrix = function () {
  ...
  this.normal = (new TONG.Vector3(0, 0, 1)).applyEuler(this.mesh.rotation);
  var cameraPosition = this.camera.position.clone().sub( this.mesh.position );
  if ( this.normal.dot(cameraPosition) < 0 ) {
    var meshNormal = (new TONG.Vector3(0, 0, 1)).applyEuler(this.mesh.rotation);
  ...
  var worldCoordinates = new TONG.Vector3();
  worldCoordinates.setFromMatrixPosition(this.camera.matrixWorld);
  this.eye = worldCoordinates;
  this.material.uniforms.eye.value = this.eye;
};

TONG.Water.prototype.render = function (isTempTexture) {
  ...
  if ( this.scene !== undefined && this.scene instanceof TONG.Scene ) {
