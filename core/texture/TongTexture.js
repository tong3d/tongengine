 ...
 * @default false
 */
function TongTexture(image, mapping, wrapS, wrapT, magFilter, minFilter, format, type, anisotropy, encoding)
{
	//If image is a URL
	if(typeof image === "string")
	{
		this.img = new TongImage(image);
	}
	else if(image === undefined)
	{
		this.img = new TongImage();
	}
	else
	{
		this.img = image;
	...
	//Super constructor
	TONG.Texture.call(this, document.createElement("img"), mapping, wrapS, wrapT, magFilter, minFilter, format, type, anisotropy, encoding);
	...
	this.category = "Image";
	this.disposed = false;
	this.format = this.img.hasTransparency() ? TONG.RGBAFormat : TONG.RGBFormat;
	...
	{
		console.log("Tong Engine: Failed to load image " + self.img.uuid + " data.");
	...
	if(this.img.encoding === "gif")
	{
		...
		this.magFilter = TONG.LinearFilter;
		this.minFilter = TONG.LinearFilter;
...
TongTexture.prototype = Object.create(TONG.Texture.prototype);
 ...
 * @method dispose
 */
TongTexture.prototype.dispose = function()
{	
 ...
 * @method toJSON
 */
TongTexture.prototype.toJSON = function(meta)
{
